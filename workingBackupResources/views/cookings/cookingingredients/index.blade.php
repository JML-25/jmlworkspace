...
<div>
	...
	<!-- Le tableau pour lister les articles/ingredientcookings -->
			...
			@foreach ($ingredientcookings as $cookingingredient)
			...
			<div class=" border-y hover:bg-green-100 bg-amber-50 flex items-center justify-between">
				<div class="p-3 px-5">
					...
					<a href="{{ route('ingredientcookings.show', $cooking) }}" title="Voir l'ingrédient">{{ $cookingingredient->title }}
				...
				<div class="p-3 px-5">
					...
					<a href="{{ route('ingredientcookings.edit', $cooking) }}" class="mr-3 text-sm bg-blue-500 hover:bg-blue-700 text-white py-1 px-2  focus:outline-none focus:shadow-outline">Modifier</a>
	...
	<p class="text-center my-5">
		...
		<a href="{{ route('ingredientcookings.create') }}" title="Créer une recette" class="mr-3 text-sm bg-blue-800 hover:bg-blue-700 text-white py-1 px-2 rounded focus:outline-none focus:shadow-outline inline-block" >Créer une nouvelle recette</a>
	</p>
	<div class="border-y pb-5">
		{{ $ingredientcookings->links('vendor.pagination.tailwindcopy2') }}
